...
import { LegacySectionBuilder } from "./LegacySectionBuilder";
import {
  PendingUnlocksPanel,
...
interface WebsiteLegacyBuilderCoreProps {
 ...
 */
export function WebsiteLegacyBuilderCore({
  identity,
  canWrite,
  businessId,
  locations,
  ...
  useEffect(() => {
    const handlePageShow = (event: PageTransitionEvent) => {
      if (event.persisted && isVariantCheckoutLoading) {
        dispatch(releaseWebsiteCheckoutBusyAction());
      }
    };
    window.addEventListener("pageshow", handlePageShow);
    return () => window.removeEventListener("pageshow", handlePageShow);
  }, [dispatch, isVariantCheckoutLoading]);
  ...
  const handleBuyVariant = (variant: WebsiteVariantCatalogEntry) => {
    if (businessId == null || checkoutReconciliationBlocked || !catalogPurchasesReady) return;
  ...
  const handleToggleCartVariant = (variant: WebsiteVariantCatalogEntry) => {
    if (checkoutReconciliationBlocked || !catalogPurchasesReady) return;
    ...
    <div className="mb-0 md:mb-8">
      ...
      >
        <LegacySectionBuilder
              ...
              <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(260px,0.82fr)_minmax(0,1.35fr)] xl:items-start">
                <div className="min-w-0">
                  <BrandingSection
                    ...
                    pageName={identity.name?.trim() || t("page.identity.fallbackName")}
                    fontKey={form.fontKey}
                  />
                </div>
                <div className="min-w-0">
          ...
          cartControl={
            canWrite && !isNative ? (
              <PendingUnlocksTrigger
                ...
                isBlocked={checkoutReconciliationBlocked || !catalogPurchasesReady}
                onRemove={(item) =>
          ...
          purchaseActionsReady={catalogPurchasesReady && !checkoutReconciliationBlocked}
...
export default WebsiteLegacyBuilderCore;
