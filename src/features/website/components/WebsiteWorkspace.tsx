...
export function WebsiteWorkspace({ identity, draft, locations, businessId }: WebsiteWorkspaceProps) {
  ...
  const form = useWebsiteDraft({ draft, lastSavedRequestId, onSave, onPublish });
  ...
  return (
    <div className="cursor-default">
      ...
      <ConfirmDialog
        ...
        onCancel={() => dispatch(clearWebsiteConflictAction())}
        onOpenChange={(open: boolean) => {
          if (!open) dispatch(clearWebsiteConflictAction());
        }}
        title={t("page.conflict.title")}
        description={t("page.conflict.description")}
        confirmTitle={t("page.conflict.reload")}
        cancelTitle={t("page.conflict.keepEditing")}
        icon={AlertTriangle}
        iconBgColor="transparent"
