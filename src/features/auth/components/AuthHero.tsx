...
/**
 * Hero panel for the login right side. One-shot wordmark reveal:
 ...
 *     primary visual). This prevents the "outline-being-traced" artifact
 *     that compound paths like 'z' produce when stroke-trace runs first.
...
// Dash starts while the wordmark is still settling (a₂ ends at 2540ms),
// so the closing punctuation overlaps the tail of the wordmark instead
// of waiting for it. Adds ~360ms past the last letter for the final beat.
const DASH_TIMING = {
  start: 2200,
  duration: 700,
}
...
// deceleration out, with the bulk of progress happening visibly across
// the middle of each letter's duration. Prevents the "snap" feel that
// strong ease-out produces.
...
// StrictMode's rapid double-mount in dev still see the animation — its
// second mount happens within ~10–20ms of the first, so we treat that as
// "the same play" rather than a real navigation.
...
export function AuthHero() {
  ...
  useLayoutEffect(() => {
    ...
    if (reducedMotion || alreadyPlayed) {
      ...
      //   revealed when the user navigates back to /login or /register
    ...
    letters.forEach((p) => {
      ...
      // fillOpacity rising both happen across the whole duration. The
      // letter's shape is visible from t=0 (just at low opacity) so the
      // viewer reads it as the letter, not as an outline being traced.
    ...
    >
      ...
      >
        <defs>
           ...
           *  and dissolves into the surface near the bottom. userSpaceOnUse
           *  keeps the gradient mapped to the viewBox, so every circle
           *  inherits the same top→bottom fade regardless of its own size. */}
          <linearGradient
      ...
      >
        <div className="auth-hero-asset">
          <svg ref={svgRef} viewBox="100 540 880 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              {/* Black wordmark glyphs — fade up on entry */}
              ...
              {/* Terracotta dash — draws in, then breathes */}
