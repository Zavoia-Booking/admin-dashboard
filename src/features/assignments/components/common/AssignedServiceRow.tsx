...
export function AssignedServiceRow({
    ...
    if (open) {
      ...
      const priceValue =
        customPrice !== null && customPrice !== undefined
          ...
          : defaultPrice ?? 0;
      const durationValue =
        customDuration !== null && customDuration !== undefined
          ? customDuration
          : defaultDuration ?? 0;
  ...
  // Check if custom pricing/duration is enabled
  const canEditCustom = !!onUpdateCustomPrice && !!onUpdateCustomDuration;
  
  // Props-based values for display outside popover
  const hasCustomPrice =
    customPrice !== null && customPrice !== (defaultPrice ?? 0);
  const hasCustomDuration =
    customDuration !== null && customDuration !== defaultDuration;
  const isCustom = canEditCustom && (hasCustomPrice || hasCustomDuration);

  // For display outside popover - use displayCustomPrice from backend if available, otherwise calculate from customPrice
  const displayPrice =
    displayCustomPrice !== null && displayCustomPrice !== undefined
      ? displayCustomPrice
      : customPrice !== null
      ? customPrice / 100
      : defaultDisplayPrice ?? 0;
  const displayDuration =
    customDuration !== null ? customDuration : defaultDuration || 0;
  ...
  const handleSave = () => {
    ...
    if (!isNaN(durationValue) && durationValue >= 0 && onUpdateCustomDuration) {
  ...
  const renderFormContent = () => (
    <>
      <div className="space-y-1.5">
        ...
        <div className="flex items-start gap-1.5 text-xs text-foreground-3 dark:text-foreground-2 leading-relaxed">
          ...
          <p>
            ...
            <span className="font-medium text-foreground-1">{serviceName}</span>{" "}
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            {hasCustomPrice && defaultDisplayValue !== null && (
              <div
                ...
                className="flex items-center w-fit rounded-full px-2.5 py-0.5 text-[11px] font-medium bg-info/20 text-foreground-3 dark:text-foreground-1 dark:bg-info/60 border border-border dark:border-border-subtle cursor-pointer hover:bg-info/30 dark:hover:bg-info/60"
              >
        ...
        <div className="space-y-1.5 mt-8 mb-2">
          {/* Label row with pill */}
          <div className="flex items-center justify-between">
            <Label
            ...
            {hasCustomDuration && defaultDuration && (
              <div
                ...
                className="flex items-center w-fit rounded-full px-2.5 py-0.5 text-[11px] font-medium bg-info/20 dark:bg-info/60 text-foreground-3 dark:text-foreground-1 border border-border dark:border-border-subtle cursor-pointer hover:bg-info/30 dark:hover:bg-info/60"
              >
          ...
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <Clock className="h-3.5 w-3.5 text-foreground-3 dark:text-foreground-2" />
            </div>
            <Input
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <span className="text-xs text-foreground-3 dark:text-foreground-2">
                {t("page.serviceRow.fields.minSuffix")}
              </span>
            </div>
          </div>
              ...
              <button
                ...
                className={`w-full px-2 py-1 text-[11px] cursor-pointer font-medium rounded-md border text-center transition-colors ${
                  Number(localDuration) === minutes
                    ...
                    : "bg-surface text-foreground-3 dark:text-foreground-2 border-border dark:border-border-subtle hover:bg-surface-hover"
                }`}
              >
                {minutes}m
              </button>
  ...
  if (isMobile) {
    return (
      <div
        className={`group flex items-start cursor-default gap-3 px-0 py-3 w-full hover:bg-surface-active dark:hover:bg-surface-hover ${
          isCustom ? "pl-0" : "pl-2"
        }`}
      >
        {isCustom ? (
          <div className="w-[3px] h-26 bg-primary/60 dark:bg-primary/80 rounded-full"></div>
        ) : null}
        <div className="flex-1 min-w-0">
          {/* First row: title + custom pill on left, edit button on right */}
          <div className="flex items-center justify-between gap-2 mb-2">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="truncate font-medium text-sm text-foreground-1">
                {serviceName}
              </div>
              {isCustom && (
                <Badge
                  variant="secondary"
                  className="text-[10px] px-1.5 py-1 bg-info/30 text-foreground-1 hover:bg-info/30 h-4 dark:bg-info/60 dark:hover:bg-info/60 rounded-full shrink-0"
                >
            ...
            {canEditCustom && (
              <Drawer open={isEditOpen} onOpenChange={handlePopoverOpenChange} autoFocus={true}>
                <DrawerTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="shrink-0 text-foreground-3 hover:text-primary group-hover:text-primary hover:bg-surface-active dark:hover:bg-surface-hover"
                  >
              ...
              >
                <DrawerTitle className="sr-only">{serviceName}</DrawerTitle>
                <DrawerDescription className="sr-only">
                  {t("page.serviceRow.popover.helperText")} {serviceName}{" "}
                  {t("page.serviceRow.popover.whenOfferedBy")}{" "}
                  {teamMemberName ||
                    t("page.serviceRow.popover.thisTeamMember")}
                  .
                </DrawerDescription>
                <div className="p-4 space-y-4">{renderFormContent()}</div>
              </DrawerContent>
            </Drawer>
            )}
          </div>
          {/* Second row: category pill, duration, and price */}
          <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
            <div className="flex items-center gap-4">
              <div className="flex items-center justify-start gap-0 text-sm font-semibold text-foreground-1 shrink-0">
                {CurrencyIcon ? (
                  <>
              ...
              <div className="inline-flex items-center justify-start gap-1 text-[11px] text-foreground-3 dark:text-foreground-2 shrink-0">
          ...
          {/* Third row: default price and duration (same as desktop) */}
          {isCustom && defaultDisplayValue !== null && (
            <div className="text-[11px] w-fit text-foreground-3 dark:text-foreground-2 border-t border-border dark:border-border pt-2">
              {t("page.serviceRow.default")} {currencyDisplay.symbol}
              <span className="text-foreground-1 ml-[2px]">
                {defaultDisplayValue.toFixed(2)}
              </span>
              <span className="text-foreground-3 dark:text-foreground-2 ml-1">
  ...
  return (
    <div
      className={`group flex items-start cursor-default gap-3 px-3 py-3.5 w-full hover:bg-surface-active dark:hover:bg-surface-hover  ${
        isCustom ? "pl-0" : ""
      }`}
    >
      {isCustom ? (
        <div className="w-[3px] h-20 bg-primary/60 dark:bg-primary/80 rounded-full"></div>
      ) : null}
      <div className="flex-1 min-w-0">
        {/* Title row */}
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Duration */}
          <div className="inline-flex items-center min-w-16 justify-start gap-1 text-[11px] text-foreground-3 dark:text-foreground-2 shrink-0">
            <Clock className="h-3 w-3" />
            <span>{formatDuration(displayDuration)}</span>
          </div>
        ...
        {isCustom && defaultDisplayValue !== null && (
          <div className="mt-2 text-[11px] w-fit text-foreground-3 dark:text-foreground-2 border-t border-border dark:border-border pt-2">
            {t("page.serviceRow.default")} {currencyDisplay.symbol}
            <span className="text-foreground-1 ml-[2px]">
              {defaultDisplayValue.toFixed(2)}
            </span>
      ...
      {canEditCustom && (
        <Popover open={isEditOpen} onOpenChange={handlePopoverOpenChange}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="shrink-0 pt-4 text-foreground-3 hover:text-primary group-hover:text-primary hover:bg-surface-active dark:hover:bg-surface-hover"
            >
              <Edit2 className="h-4 w-4" />
            </Button>
          </PopoverTrigger>
        ...
        >
          <button
            ...
            className="absolute top-2 right-0 rounded-md text-foreground-3 dark:text-foreground-2 hover:text-primary cursor-pointer"
