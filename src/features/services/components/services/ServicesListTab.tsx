...
import {
  ItemCard,
  type ItemCardMetadata,
  type ItemCardBadge,
} from "../../../../shared/components/common/ItemCard";
...
export function ServicesListTab({ isActive = true }: ServicesListTabProps) {
  ...
  const applyCategoryChanges = async (): Promise<void> => {
    ...
    const updated: ServiceCategory[] = categories.filter((cat) => {
      ...
      return (
        initial.name !== cat.name || (initial.color || "") !== (cat.color || "")
      );
    });
  ...
  const hasActiveFilters = useMemo(() => {
    ...
    return (
      hasSearch || hasPriceFilter || hasDurationFilter || hasCategoryFilter
    );
  }, [filters]);
  ...
  const isFilteredEmpty =
    isEmptyState && !hasNoServicesAtAll && hasActiveFilters;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {services.map((service) => {
                ...
                const locationsCount =
                  service.locations?.length ?? service.locationsCount ?? 0;
                if (locationsCount > 0) {
                  badges.push({
                    label:
                      locationsCount === 1
                ...
                const teamMembersCount =
                  service.teamMembers?.length ?? service.teamMembersCount ?? 0;
