...
export const Pill = React.forwardRef<HTMLButtonElement, PillProps>(
  ({ selected = false, icon: Icon, logo, showCheckmark = false, children, className, ...props }, ref) => {
    return (
      <button
        ref={ref}
        type="button"
        className={cn(
          'group relative flex items-center justify-center min-h-14 px-3 py-2 rounded-lg border text-sm font-medium transition-colors cursor-pointer overflow-visible',
        ...
        {...props}
      >
        <span className="flex items-center leading-tight select-none w-full">
          {logo && <span className="mr-1.5 shrink-0">{logo}</span>}
          {!logo && Icon && <Icon className="h-4 w-4 mr-1.5 shrink-0" />}
          {children}
        </span>
        {showCheckmark && (
          <div className={cn(
