...
const CustomTimePicker: React.FC<CustomTimePickerProps> = ({ id, label, value, onChange, min, max, stepMinutes }) => {
  ...
  const handleTriggerClick = useCallback(() => setIsOpen(true), []);
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      <span id={`${id}-label`} className="text-[11px] text-gray-400 whitespace-nowrap">
        {label}
      </span>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <span
            ...
            className="inline-flex !h-8 !min-h-0 px-4 rounded-full bg-gray-100 items-center justify-center hover:bg-gray-200 w-[92px] text-sm leading-none cursor-pointer transition-colors select-none"
        ...
        <PopoverContent 
          className="w-[280px] p-3 rounded-lg border border-gray-200 bg-white shadow-md" 
          align="start" 
          side="bottom" 
          sideOffset={8}
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="mb-1 text-sm font-medium text-gray-700">{headerText}</div>
            <div className="flex items-start justify-center gap-2">
              {/* Hour selector */}
              <div className="flex flex-col items-center w-28" role="listbox" aria-label="Select hour">
                <div className="text-[11px] text-gray-500 mb-1 font-medium">Hour</div>
                <div 
                  ref={hourScrollRef}
                  className="relative h-36 w-full overflow-y-auto scroll-smooth border border-gray-200 rounded-md bg-white tabular-nums px-1 py-1"
                ...
                >
                  {hours.map((h) => (
                    <div
                      ...
                      className={`h-8 !min-h-0 w-full my-0.5 text-sm leading-none flex items-center justify-center transition-colors cursor-pointer select-none rounded-full tabular-nums ${
                        localHour === h
              ...
              <div className="flex flex-col items-center w-28" role="listbox" aria-label="Select minute">
                <div className="text-[11px] text-gray-500 mb-1 font-medium">Minute</div>
                <div 
                  ref={minuteScrollRef}
                  className="relative h-36 w-full overflow-y-auto scroll-smooth border border-gray-200 rounded-md bg-white tabular-nums px-1 py-1"
                  style={{ scrollbarWidth: 'thin' }}
            ...
            {/* AM/PM selector */}
              <div className="flex flex-col items-center w-20" role="listbox" aria-label="Select period">
                <div className="text-[11px] text-gray-500 mb-1 font-medium">Period</div>
                <div className="flex flex-col gap-2 w-full h-36 justify-center">
                  <div
                    role="option"
                    ...
                    className={`h-8 !min-h-0 inline-flex items-center justify-center px-4 rounded-full text-sm font-medium transition-colors cursor-pointer select-none tabular-nums ${
                      localPeriod === 'AM'
            ...
            <button
              type="button"
              onClick={handleDone}
              className="sticky bottom-0 h-11 inline-flex items-center justify-center px-4 bg-black text-white rounded-md hover:bg-gray-800 transition-colors font-medium text-sm cursor-pointer"
            >
              Done
            </button>
          </div>
